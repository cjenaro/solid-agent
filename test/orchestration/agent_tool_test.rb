...
class AgentToolTest < ActiveSupport::TestCase
  ...
  test 'marks span as error on failure and re-raises' do
    failing_agent = Class.new do
      ...
      def self.perform_now(_input, conversation:)
  ...
  test 'creates span with OTel execute_tool attributes' do
    @tool.execute(
      { 'input' => 'text' },
      context: { trace: @trace, conversation: @conversation }
    )
    ...
    span = @trace.spans.last
    assert_equal 'execute_tool', span.metadata['gen_ai.operation.name']
    assert_equal 'quick_summary', span.metadata['gen_ai.tool.name']
    assert_equal 'agent', span.metadata['gen_ai.tool.type']
  end
end
